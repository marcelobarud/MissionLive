...
export class ActivityService {
  ...
  async record(actorUserId: string, eventType: string, resource: { goalId?: string; teamId?: string; targetUserId?: string }, metadata: Metadata = {}, client?: Prisma.TransactionClient) {
    const prisma = client ?? this.prisma;
    const event = await prisma.activityEvent.create({ data: { actorUserId, eventType, goalId: resource.goalId, teamId: resource.teamId, targetUserId: resource.targetUserId, metadataJson: JSON.stringify(metadata) } });
    const recipients = new Set<string>(resource.targetUserId ? [resource.targetUserId] : []);
    if (resource.goalId) {
      const goal = await prisma.goal.findUnique({ where: { id: resource.goalId }, select: { ownerUserId: true, members: { select: { userId: true } }, team: { select: { ownerUserId: true, members: { select: { userId: true } } } } } });
      ...
      const team = await prisma.team.findUnique({ where: { id: resource.teamId }, select: { ownerUserId: true, members: { select: { userId: true } } } });
    ...
    if (notify.length) await prisma.notification.createMany({ data: notify.map((userId) => ({ userId, type: eventType, title: 'Atualização em uma colaboração', body: `Você recebeu uma atualização de ${eventType.replaceAll('_', ' ')}.`, goalId: resource.goalId, teamId: resource.teamId })) });
    return event;
  }
