...
export class GoalsService {
  ...
  async rhythm(userId: string, goalId: string) {
    ...
    if (!end || !goal.steps.length) return { applicable: false, state: 'unavailable', reason: 'A meta precisa ter início, prazo e pelo menos um passo.' };
    const elapsed = Math.min(100, Math.max(0, (now - start) / Math.max(1, end - start) * 100)); const done = goal.steps.filter((step) => step.progresses?.some((progress) => progress.userId === userId && progress.completed)).length; const progress = done / goal.steps.length * 100; const state = goal.status === 'completed' ? 'completed' : now > end && progress < 100 ? 'late' : progress >= elapsed + 10 ? 'ahead' : progress + 10 < elapsed ? 'attention' : 'on-track';
