...
export class GoalsService {
  ...
  async remove(userId: string, goalId: string) { const access = await this.role(userId, goalId); if (access.role !== 'owner' && !MANAGE_ROLES.has(access.role)) throw new ForbiddenException('You cannot remove this goal.'); await this.prisma.goal.update({ where: { id: goalId }, data: { status: 'cancelled' } }); return { cancelled: true }; }
  async archive(userId: string, goalId: string) { const access = await this.role(userId, goalId); if (access.role !== 'owner' && !MANAGE_ROLES.has(access.role)) throw new ForbiddenException('Only the owner or an admin can archive this goal.'); await this.prisma.goal.update({ where: { id: goalId }, data: { status: 'archived' } }); return { archived: true }; }

  async addStep(userId: string, goalId: string, dto: CreateStepDto) { const access = await this.role(userId, goalId); if (access.role !== 'owner' && !EDITABLE_ROLES.has(access.role)) throw new ForbiddenException('You cannot edit steps.'); const last = await this.prisma.goalStep.aggregate({ where: { goalId }, _max: { position: true } }); await this.prisma.goalStep.create({ data: { goalId, title: dto.title.trim(), description: dto.description?.trim() || undefined, position: (last._max.position ?? -1) + 1 } }); return this.get(userId, goalId); }
  async updateStep(userId: string, goalId: string, stepId: string, dto: UpdateStepDto) { const access = await this.role(userId, goalId); if (access.role !== 'owner' && !EDITABLE_ROLES.has(access.role)) throw new ForbiddenException('You cannot edit steps.'); const step = await this.prisma.goalStep.findFirst({ where: { id: stepId, goalId } }); if (!step) throw new NotFoundException('Step not found.'); await this.prisma.goalStep.update({ where: { id: stepId }, data: { title: dto.title.trim(), description: dto.description?.trim() || null } }); return this.get(userId, goalId); }
  ...
  async reorderSteps(userId: string, goalId: string, dto: ReorderStepsDto) { const access = await this.role(userId, goalId); if (access.role !== 'owner' && !EDITABLE_ROLES.has(access.role)) throw new ForbiddenException('You cannot edit steps.'); const steps = await this.prisma.goalStep.findMany({ where: { goalId }, select: { id: true } }); if (steps.length !== dto.stepIds.length || new Set(dto.stepIds).size !== steps.length || steps.some((step) => !dto.stepIds.includes(step.id))) throw new BadRequestException('stepIds must contain each goal step exactly once.'); await this.prisma.$transaction(async (tx) => { for (const [index, id] of dto.stepIds.entries()) await tx.goalStep.update({ where: { id }, data: { position: index + 100000 } }); for (const [index, id] of dto.stepIds.entries()) await tx.goalStep.update({ where: { id }, data: { position: index } }); }); return this.get(userId, goalId); }
  async setProgress(userId: string, goalId: string, stepId: string, dto: ProgressDto) { await this.role(userId, goalId); const step = await this.prisma.goalStep.findFirst({ where: { id: stepId, goalId } }); if (!step) throw new NotFoundException('Step not found.'); await this.prisma.goalStepProgress.upsert({ where: { goalStepId_userId: { goalStepId: stepId, userId } }, update: { completed: dto.completed, completedAt: dto.completed ? new Date() : null }, create: { goalStepId: stepId, userId, completed: dto.completed, completedAt: dto.completed ? new Date() : null } }); await this.recalculate(goalId); return this.get(userId, goalId); }
