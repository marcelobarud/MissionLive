...
describe('AuthGuard', () => {
  it('aceita somente sessão ativa, não revogada e não expirada', async () => {
    const { request, execution } = context(); const prisma = { session: { findFirst: jest.fn().mockResolvedValue({ id: 'session-1', user: { id: 'user-1', email: 'ana@example.com', name: 'Ana', status: 'active', platformRole: 'ADMIN', avatarUrl: null, avatarType: null, avatarPresetId: null, avatarFileKey: null, timezone: 'UTC', onboardingCompletedAt: null, preferencesJson: '{}' } }) } };
    ...
    expect((request.user as { platformRole: string }).platformRole).toBe('ADMIN');
  ...
  it.each([
    ...
    { label: 'conta desabilitada', session: { id: 'session-1', user: { id: 'user-1', email: 'ana@example.com', name: 'Ana', status: 'disabled', platformRole: 'SUPER_ADMIN', avatarUrl: null, avatarType: null, avatarPresetId: null, avatarFileKey: null, timezone: 'UTC', onboardingCompletedAt: null, preferencesJson: '{}' } } },
