...
export function GoalGallery({ goal }: { goal: Goal }) {
  ...
  return <>{gallery}{addDialog()}<Dialog open={Boolean(selected)} title={selected?.title ?? 'Foto'} onClose={() => setSelected(null)}>{selected && <div className="goal-photo-detail"><img src={api.mediaUrl(selected.imageUrl)} alt={selected.title} width="1920" height="1280" crossOrigin="use-credentials" /><div><p>{selected.description}</p><small>{selected.author.name} · {photoDate(selected)}{selected.occurrenceLocalDate ? ` · ${selected.occurrenceLocalDate}` : ''}</small>{selected.task && <small>Tarefa: {selected.task.title}{selected.task.removed ? ' · removida' : ''}</small>}</div>{selected.canDelete && <Button variant="danger" type="button" startIcon={<IconTrash size={16} stroke={1.9} />} onClick={() => { void deletePhoto(selected); }}>Excluir foto</Button>}</div>}</Dialog></>;
}
