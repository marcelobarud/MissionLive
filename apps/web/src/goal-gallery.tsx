...
export function GoalGallery({ goal }: { goal: Goal }) {
  ...
  return <>{gallery}{addDialog()}<Dialog open={Boolean(selected)} title="Visualização da foto" className="photo-dialog" onClose={() => setSelected(null)}>{selected && <div className="goal-photo-detail"><img src={api.mediaUrl(selected.imageUrl)} alt={selected.title} width="1920" height="1280" crossOrigin="use-credentials" /><div className="goal-photo-detail-info"><h3>{selected.title}</h3><p>{selected.description}</p><div className="goal-photo-detail-meta"><span>{selected.author.name}</span><span>{photoDate(selected)}{selected.occurrenceLocalDate ? ` · ${selected.occurrenceLocalDate}` : ''}</span></div>{selected.task && <span className="goal-photo-detail-task">Tarefa: {selected.task.title}{selected.task.removed ? ' · removida' : ''}</span>}</div>{selected.canDelete && <div className="goal-photo-detail-actions"><Button variant="danger" type="button" startIcon={<IconTrash size={16} stroke={1.9} />} onClick={() => { void deletePhoto(selected); }}>Excluir foto</Button></div>}</div>}</Dialog></>;
}
