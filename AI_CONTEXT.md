...
- `GET /goals` aplica owner/membership/equipe também à contagem e retorna `{ items, pagination }`; aceita `page` (mínimo 1, padrão 1), `pageSize` (1–50, padrão 12; clamp ao limite e à última página), preserva filtros server-side e usa ordenação estável por `id` como desempate. Ordenação por progresso calcula a classificação global com dados mínimos e hidrata somente a página selecionada;
...
- listagem de metas com busca, filtros, ordenação e paginação server-side; página e filtros refletem a query string, a tela usa 12 itens por página, controles acessíveis de anterior/próxima e reinicia na página 1 ao alterar filtros/ordenação; criação completa e edição inicial de metas, detalhe com datas/tags/contexto/participantes/progresso, progresso individual expandível por participante em modo somente leitura, CRUD de steps, cancelamento, arquivamento, exclusão permanente protegida por owner, owner override e convite direto de meta;
